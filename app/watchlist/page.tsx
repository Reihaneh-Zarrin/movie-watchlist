...
export default async function WatchlistPage() {
  ...
  const movies: SavedMovies | undefined = movieDb.findUserWatchlist(user.id);

  const movieDetail: (Movie | { error: string })[] = await Promise.all(
    movies?.movieId.map((id) => getMovieById(id)) || [],
  );

  const successfulMovies = movieDetail.filter(
    (movie): movie is Movie => !("error" in movie),
  );

  const hasError = movieDetail.some((movie) => "error" in movie);

  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Your Watchlist</h1>

      {hasError && <p>Could not load some of the movies. Please try again.</p>}
   
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6 mx-4 p-4">
          {successfulMovies.map((movie, index) => (
            <li
              key={movie.id}
